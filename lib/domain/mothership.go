...
func (d *Domain) CreateMothership(m dfn.Mothership,ctx context.Context) error {
  ...
  if err !=  nil{
    d.LogToFile(utils.Logger{Name:"ms_sql",Text:fmt.Sprintf("Error preparing to create mothership: %s",err),})
    return errors.New("Server encountered an error while preparing to create mothership. Try again later :).")
  }
  defer ins.Close()
  res,err := ins.ExecContext(ctx,&m.OwnerID,&m.Name,&m.Password,&m.MSId,&m.Address,&m.ImplantTunnel,&m.AdminTunnel,&m.Motherships,&m.Description,&m.Tls,&m.CertPem,&m.KeyPem,&m.Active,&m.GenCommand,&m.Machinedata,&m.CreatedAt,&m.UpdatedAt)
  rowsAffec, _  := res.RowsAffected()
  if err != nil || rowsAffec != 1{
    d.LogToFile(utils.Logger{Name:"ms_sql",Text:fmt.Sprintf("Error excuting create mothership: %s",err),})
...
func (d *Domain) ListMotherShips(active bool,ctx context.Context) ([]dfn.Mothership,error) {
  ...
  if err != nil{
    d.LogToFile(utils.Logger{Name:"ms_sql",Text:fmt.Sprintf("Error listing mothership: %s",err),})
  ...
  for rows.Next(){
    ...
    err = rows.Scan(&m.OwnerID,&m.Name,&m.Password,&m.MSId,&m.Address,&m.ImplantTunnel,&m.AdminTunnel,&m.Motherships,&m.Description,&m.Tls,&m.CertPem,&m.KeyPem,&m.Active,&m.GenCommand,&m.Machinedata,&m.CreatedAt,&m.UpdatedAt)
    if err != nil{
      d.LogToFile(utils.Logger{Name:"ms_sql",Text:fmt.Sprintf("Error scanning for mothership: %s",err),})
...
func (d *Domain) DeactivateMothership(msid,ownerId string,ctx context.Context) error {
  ...
  if err != nil{
    d.LogToFile(utils.Logger{Name:"ms_sql",Text:fmt.Sprintf("Error preparing to deactivate mothership: %s",err),})
  ...
  rowsAffec,_ := res.RowsAffected()
  if err != nil || rowsAffec != 1 {
    d.LogToFile(utils.Logger{Name:"ms_sql",Text:fmt.Sprintf("Error executing deactivate mothership: %s",err),})
...
func (d *Domain) ViewMothership(msid string,ctx context.Context)(*dfn.Mothership,error){
  ...
  err = row.Scan(&m.OwnerID,&m.Name,&m.Password,&m.MSId,&m.Address,&m.ImplantTunnel,&m.AdminTunnel,&m.Motherships,&m.Description,&m.Tls,&m.CertPem,&m.KeyPem,&m.Active,&m.GenCommand,&m.Machinedata,&m.CreatedAt,&m.UpdatedAt)
  if err != nil{
    d.LogToFile(utils.Logger{Name:"ms_sql",Text:fmt.Sprintf("Error viewing mothership: %s",err),})
