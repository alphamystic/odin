...
func (d *Domain) CreateAsset(a dfn.Asset,ctx context.Context) error {
  ...
  if err !=  nil{
    d.LogToFile(utils.Logger{Name:"assets_sql",Text:fmt.Sprintf("Error preparing to create asset: %s",err),})
...
func (d *Domain)  ListAssets(active bool,ctx context.Context)([]dfn.Asset,error){
  conn,err := d.GetConnection(ctx)
  if err != nil {
  ...
  if err != nil{
    d.LogToFile(utils.Logger{Name:"assets_sql",Text:fmt.Sprintf("EQA: %s",err),})
  ...
  for rows.Next(){
    ...
    if err != nil{
      d.LogToFile(utils.Logger{Name:"assets_sql",Text:fmt.Sprintf("Error scanning for assets: %s",err),})
...
func (d *Domain)  ViewAsset(assetId string,ctx context.Context)(*dfn.Asset,error){
  conn,err := d.GetConnection(ctx)
  if err != nil {
    return nil,fmt.Errorf("Error getting db connection: %q",err)
  }
  ...
  if err != nil{
    d.LogToFile(utils.Logger{Name:"assets_danger_sql",Text:fmt.Sprintf("Error viewing asset with id %s.ERROR: %s",assetId,err),})
