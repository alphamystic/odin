...
func (d *Domain) Authenticate(ctx context.Context,password,email string)  (*dfn.User,error){
  ...
  if err != nil{
    ...
    d.LogToFile(utils.Logger{Name: "auth_sql", Text: fmt.Sprintf("A non-user with email %s tried accessing the server", email)})
    return &dfn.User{},err
  }
  if !active{
    d.LogToFile(utils.Logger{Name: "auth_danger_sql", Text: fmt.Sprintf("A non-active with email %s tried accessing the server", email),})
    return &dfn.User{},dfn.NonActiveUser
  }
  err = utils.CheckPasswordHash(password, hash)
  if err != nil{
    d.LogToFile(utils.Logger{Name: "auth_danger", Text: fmt.Sprintf("Wrong login attempt for email %s with password %s  %s", email, password, err),})
