...
// Builder serves as the single centralized structure configuration for the package
type Builder struct {
	Name         string
	Architecture string
	OsType       string
...
func (b *Builder) CreateBuildCommand() error {
	...
	case "macbin":
		...
		b.BuildCommand = buf.String()
	default:
		return fmt.Errorf("Unsupported format. We are in build this actually shouldn't happen unless it's a file type format")
	}
	return nil
}
...
func (b *Builder) SetFormat(fmrt string) error {
	...
	case "go-plugin":
		b.Format = "gplgn"
	default:
		return errors.New("Unsupported payload format or not yet implemented")
	}
	return nil
}
...
func (b *Builder) SetEncoding(enc string) error {
	...
	case "basic":
		b.Encoding = "basic"
	case "donut":
		// FIXED: Removed the stray space inside the string comparison '" windows"'
		if b.OsType != "windows" {
...
var SupportedArch = func() {
	var supportedArch = `GOOS=aix GOARCH=ppc64
